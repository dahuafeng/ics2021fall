 ...
 * 2000012965 凤大骅
 * 
 * 函数eval用于对命令行求值，它首先调用parseline函数，这个函数可以
 * 解析以空格分割的命令行参数，解析的结果存在tok中，其中包含命令行的
 * 参数个数argc，参数argv[]，I/O重定向文件以及命令类型；解析完后，
 * eval函数调用builtin_command函数，用于判断是否是内置命令：如果是
 * 则立刻执行，并返回1，否则返回0；如果不是内置命令，那么shell创建一
 * 个子进程，并在子进程中执行命令；如果是后台执行，则shell回到循环开始；
 * 如果是前台执行，shell就调用sigsuspend等待子进程结束；
 * 
 * handlers：
 * sigchld handler：处理SIGCHLD信号，在这个函数中回收子进程；
 * sigint  handler：处理SIGINT信号；
 * sigtstp handler：处理SIGTSTP信号；
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
void eval(char *cmdline)
{
    ...
    sigset_t mask_all, mask_three, prev_three;
    ...
    if (!builtin_command(tok))
    {
        ...
        //not built-in commands,fork() and execve();
        if (0 == (pid = fork()))
        {
        ...
        if (!bg)
        {
            sigset_t _mask, _mask_three, _prev;
            //for sigsuspend;
            sigfillset(&_prev);
            sigdelset(&_prev, SIGCHLD);
            sigdelset(&_prev, SIGINT);
            sigdelset(&_prev, SIGTSTP);
            //block three signals;
            sigemptyset(&_mask_three);
            sigaddset(&_mask_three, SIGCHLD);
            sigaddset(&_mask_three, SIGINT);
            sigaddset(&_mask_three, SIGTSTP);
            sigprocmask(SIG_BLOCK, &_mask_three, &_mask);
...
/* 
 * builtin_command function mainly deals with the built-in commands.
 * Similarly,this function also does not reap any of its children 
 * but just wait for the relevant signals; 
 *
 */
int builtin_command(struct cmdline_tokens tok)
{
    ...
    if (tok.builtins == BUILTIN_BG)
    {
        ...
        job->state = BG; //run in the background;
    ...
    if (tok.builtins == BUILTIN_FG)
    {
        ...
        job->state = FG; //run in the foreground;
    ...
    if (tok.builtins == BUILTIN_KILL)
    {
        struct job_t *job;
        if (tok.argv[1][0] == '%') //JID;
            job = getjobjid(job_list, abs(atoi(tok.argv[1] + 1)));
        else //PID;
            job = getjobpid(job_list, abs(atoi(tok.argv[1])));

        if (job)
            kill(-(job->pid), SIGTERM); //send SIGTERM;
...
void sigchld_handler(int sig)
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0)
    {
        ...
        //child terminates because of the uncaptured signal;
