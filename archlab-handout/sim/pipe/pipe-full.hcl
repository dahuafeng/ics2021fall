...
# 1.添加iaddq指令;
# 2.修改分支预测策略：如果跳转地址小于当前PC，则总是预测跳转；否则，预测不跳转；
# 3.修改流水线避免控制冒险的逻辑：
#	对于这段程序：
#		andq %r8,%r8
#		jle Npos		#Line 1
#		iaddq $1,%rax		#Line 2
# 	Npos:
#		rmmovq %r9,8(%rsi)	#Line 3
#	当程序执行到Line1时，预测不跳转，继续向下执行；当Line1指令进入执行(Excute)阶段时，如果
#	发现预测错误，此时Line2处于译码(Decode)阶段，这是一条错误指令，因此应该在E中插入气泡;此时#	Line3处于取指(Fetch)阶段，虽然预测错误，但Line3仍然是一条正确指令，因此D中不需要插入气泡;
#	也就是说只需要将条件跳转指令和跳转目标指令之间的指令替换为bubble，跳转目标指令及之后无需处理;
...
word f_predPC = [
	f_icode == IJXX && f_ifun != 0
	 && f_valC < f_valP : f_valC;
	f_icode == IJXX && f_ifun != 0
	 && f_valC > f_valP : f_valP;
